...
package issue_pkg;
    ...
    localparam MAX_EXEC_CYCLE_DELAY = MAX_EXEC_CYCLE_V2-1;


    typedef struct packed {
        logic valid;
        EX_MEM_TYPE funct_unit;
    ...
    typedef struct packed {
        ...
        // added
        logicn [$clog2(ROB_COUNT)-1:0] rob_ptr;
    } wb_phys_reg_pkt_t;

    typedef struct packed {
    logic wr_en;
    // logic [$clog2(MAX_PC_INSTR_BUFF)-1:0] wr_ptr;
    logic [$clog2(MAX_PC_INSTRS)-1:0] wr_ptr;
...
function automatic logic [DATA_WIDTH-1:0] format_20b_to_datawidth(
    ...
    case (opcode)
        ...
        LUI_OP, AUIPC_OP: begin
            return DATA_WIDTH'($signed({imm_20b, 12'b0}));
        end
        // J-Type: JAL
        // 7'b110_1111: begin
        JAL_OP: begin
            return DATA_WIDTH'($signed({imm_20b, 1'b0}));
        end
        // I, S, B, Z Types
        default: begin
            return DATA_WIDTH'($signed(imm_20b));
        end
    endcase
endfunction
...
function automatic EX_MEM_TYPE get_ex_mem_type(
    ...
    if (instr_valid) begin
        case (opcode)
            ALU_OP: return ALU; // R-Type
            ALU_IMM_OP: return ALU; // I-Type ALU
            LOAD_OP: return MEM; // Load
            STORE_OP: return MEM; // Store
            BRANCH_OP: return BRANCH; // Branch
            // 7'b110_1111: return ALU; // JAL
            JAL_OP: return JALR; // JAL
            JALR_OP: return JALR; // JALR
            LUI_OP: return ALU; // LUI
            AUIPC_OP: return AUIPC; // AUIPC
...
function automatic logic [EX_MEM_TYPE_SIZE-1:0] get_ex_mem_type_one_hot(
    ...
    EX_MEM_TYPE instr_type = get_ex_mem_type(opcode, instr_valid);
    return 1 << instr_type; // Convert enum to one-hot encoding
endfunction
...
function automatic logic [FUNCT_COMB_WIDTH-1:0] get_funct_comb(
    ...
    if (instr_type == JALR) begin: Jalr
    ...
    end else if (opcode == LUI_OP) begin: Lui
        return 4'b1111;
        ...
        if (opcode == ALU_IMM_OP && funct3 != 3'b101) begin: AluInotSri
            return {1'b0, funct3};
        end else begin: Default
            return {funct7[5], funct3};
        end
